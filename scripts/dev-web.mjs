...
// A forced terminal close can bypass Node's signal handlers. Reclaim only
// abandoned workspaces from this script's private prefix on the next launch.
for (const entry of readdirSync(tmpdir(), { withFileTypes: true })) {
  if (!entry.isDirectory() || !entry.name.startsWith("personal-workbench-dev-"))
    continue;
  const marker = join(tmpdir(), entry.name, ".dev-web.pid");
  let pid = 0;
  try {
    pid = Number.parseInt(readFileSync(marker, "utf8"), 10);
  } catch {
    // Workspaces from older versions have no marker and are safe to reclaim.
  }
  if (pid > 0) {
    try {
      process.kill(pid, 0);
      ...
      // The recorded process is gone; reclaim the abandoned workspace.
    }
  }
  try {
    rmSync(join(tmpdir(), entry.name), { recursive: true, force: true });
  } catch {
    // An active sidecar still owns the workspace lock; leave it untouched.
  }
}

const workspace = mkdtempSync(join(tmpdir(), "personal-workbench-dev-"));
writeFileSync(join(workspace, ".dev-web.pid"), `${process.pid}\n`, {
  encoding: "utf8",
  mode: 0o600,
});
...
backend.stdin.end(
  `${JSON.stringify({
    ...
    workspaceName: "Dev Web Workspace",
    appVersion,
...
function cleanupWorkspace() {
  try {
    ...
    console.error(`Failed to clean temporary workspace: ${error.message}`);
  }
}
...
backend.once("exit", (code) => {
  if (!stopping) {
    if (code !== 0)
      console.error(`workbenchd exited unexpectedly with code ${code}`);
    void stop(code ?? 1);
  }
});
