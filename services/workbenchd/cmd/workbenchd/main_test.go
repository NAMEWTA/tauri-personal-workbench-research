...
func TestReadBootstrap(t *testing.T) {
	valid := `{"protocolVersion":3,"parentPid":1234,"token":"0123456789012345678901234567890123456789012","workspacePath":".","workspaceName":"测试","appVersion":"0.2.11","allowedOrigins":["http://127.0.0.1:1420"]}` + "\n"
...
func TestReadBootstrapAcceptsTauriOrigins(t *testing.T) {
	valid := `{"protocolVersion":3,"parentPid":1234,"token":"0123456789012345678901234567890123456789012","workspacePath":".","workspaceName":"测试","appVersion":"0.2.11","allowedOrigins":["tauri://localhost","http://tauri.localhost","https://tauri.localhost"]}` + "\n"
...
func TestReadBootstrapRejectsRemoteAllowedOrigins(t *testing.T) {
	for _, origin := range []string{
		...
		input := `{"protocolVersion":3,"parentPid":1234,"token":"0123456789012345678901234567890123456789012","workspacePath":".","workspaceName":"测试","appVersion":"0.2.11","allowedOrigins":["` + origin + `"]}` + "\n"
		if _, err := readBootstrap(strings.NewReader(input)); err == nil {
