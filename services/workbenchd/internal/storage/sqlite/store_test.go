...
func openStore(t *testing.T) *workbenchsqlite.Store {
	t.Helper()
	store, err := workbenchsqlite.Open(context.Background(), t.TempDir(), "V2 测试", "0.2.12")
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenCreatesV2BaselineAndLocksWorkspace(t *testing.T) {
	...
	workspace := t.TempDir()
	store, err := workbenchsqlite.Open(ctx, workspace, "V2 工作区", "0.2.12")
	if err != nil {
		t.Fatal(err)
	}
	if _, err := workbenchsqlite.Open(ctx, workspace, "V2 工作区", "0.2.12"); err == nil {
...
func TestWorkspaceDataIsIsolatedAcrossSQLiteFiles(t *testing.T) {
	...
	storeA, err := workbenchsqlite.Open(ctx, workspaceA, "工作区 A", "0.2.12")
	if err != nil {
		t.Fatal(err)
	}
	defer func() { _ = storeA.Close() }()
	storeB, err := workbenchsqlite.Open(ctx, workspaceB, "工作区 B", "0.2.12")
	if err != nil {
		t.Fatal(err)
	}
...
func TestWorkspacePreferencesPersistAcrossReopen(t *testing.T) {
	ctx := context.Background()
	workspace := t.TempDir()
	store, err := workbenchsqlite.Open(ctx, workspace, "偏好测试", "0.2.12")
	if err != nil {
		t.Fatal(err)
	}
	...
	reopened, err := workbenchsqlite.Open(ctx, workspace, "偏好测试", "0.2.12")
	if err != nil {
		t.Fatal(err)
	}
